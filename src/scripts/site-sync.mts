...
function validateSiteConfig(value: unknown): SiteConfig {
  if (!isRecord(value)) throw new Error("site.config.json must be an object");
  ...
  if (!isRecord(config.marketplace)) throw new Error("marketplace must be an object");
  ...
  if (!Array.isArray(config.endpoints) || config.endpoints.length === 0) throw new Error("endpoints must be a non-empty array");
  for (const [index, endpoint] of config.endpoints.entries()) {
    if (!isRecord(endpoint)) throw new Error(`endpoints[${index}] must be an object`);
  ...
  if (!isRecord(config.cloudflare)) throw new Error("cloudflare must be an object");
...
function createSiteConfigSource(config: SiteConfig): string {
...
async function syncClaudeManifest(config: SiteConfig): Promise<void> {
...
async function syncCodexManifest(config: SiteConfig): Promise<void> {
...
async function syncClaudeMarketplace(config: SiteConfig): Promise<void> {
...
async function syncCodexMarketplace(config: SiteConfig): Promise<void> {
...
async function syncWrangler(path: string, workerName: string, d1Name: string): Promise<void> {
  ...
  next = replaceRequired(
    ...
    `${path} D1 database_name`
  );
  await writeIfChanged(path, next);
}

async function syncStatusPageHtml(config: SiteConfig): Promise<void> {
  const path = join(repoRoot, "status-page", "index.html");
  const current = await readFile(path, "utf8");
  let next = replaceRequired(
    current,
    /<title data-site-sync="title">.*?<\/title>/,
    `<title data-site-sync="title">${escapeHtml(config.siteName)} 状态</title>`,
    "status page title"
  );
  next = replaceRequired(
    next,
    /<h1 id="siteTitle">.*?<\/h1>/,
    `<h1 id="siteTitle">${escapeHtml(config.siteName)} 状态</h1>`,
    "status page h1"
  );
  next = replaceRequired(
    next,
    /<p id="siteSubtitle">.*?<\/p>/,
    `<p id="siteSubtitle">Claude Code / Codex 社区轮次观测</p>`,
    "status page subtitle"
  );
  next = replaceRequired(
    next,
    /const key = "[^"]+-theme";/,
    `const key = "${escapeJsString(config.marketplace.name)}-theme";`,
    "status page theme key"
  );
...
function replaceAnchorHref(source: string, marker: string, href: string): string {
  const pattern = new RegExp(`(<a\\b(?=[^>]*data-site-sync="${marker}")[^>]*\\bhref=")[^"]*(")`);
  if (!pattern.test(source)) throw new Error(`status page ${marker} link marker not found`);
...
function replaceRequired(source: string, pattern: RegExp, replacement: string, label: string): string {
  if (!pattern.test(source)) throw new Error(`${label} marker not found`);
...
function siteKeyword(config: SiteConfig): string {
