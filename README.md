...
AnyRouter 公益站社区状态监测 MVP。它不代理请求，不主动探测，只在用户真实使用 Claude Code 后，由插件匿名上报白名单状态字段。
...
插件只检查当前进程的 `ANTHROPIC_BASE_URL` host 是否命中这两个入口。上报时不包含实际 URL、prompt、response、token、cookie、key、账号、`session_id`、文件路径、完整日志或精确时间戳。
...
Claude Code hook 会在 `UserPromptSubmit` 记录本轮开始时间，在 `Stop` / `StopFailure` 后尝试匿名上报。只有本轮开始和结束时都命中 AnyRouter 入口才会上报。
