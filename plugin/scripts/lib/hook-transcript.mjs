...
import { classifyModel } from "./policy.mjs";
...
export async function getTranscriptSize(input) {
...
export function getTranscriptPath(input) {
    const value = input.transcript_path ?? input.transcriptPath;
    if (typeof value !== "string" || value.trim() === "")
        return null;
    return value;
}
export async function inspectPromptStartTranscript(input, transcriptStartOffset) {
    const transcriptPath = getTranscriptPath(input);
    const result = {
        inspected: false,
        modelClass: "unknown",
...
export async function inspectRecentProjectModelSwitch(input, beforeMs = Date.now()) {
    const result = {
        inspected: false,
        modelClass: "unknown",
        timestampMs: null,
    ...
    if (!isTaskNotificationInput(input))
        return result;
    const transcriptPath = getTranscriptPath(input);
    if (!transcriptPath)
        return result;
...
export async function inspectTranscript(input, turnStartedAtMs, transcriptStartOffset) {
    const transcriptPath = getTranscriptPath(input);
    const result = {
...
export function collectModelCandidates(value, prefix) {
    if (!isRecord(value))
        return [];
    const result = [];
    const seen = new Set();
...
async function listRecentProjectTranscripts(projectDir, beforeMs) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (!info.isFile())
                continue;
            if (info.size > PROJECT_MODEL_SWITCH_MAX_FILE_BYTES)
                continue;
...
async function inspectModelSwitchesInFile(path, beforeMs) {
    let latest = null;
    ...
    try {
        for await (const line of lines) {
            ...
            const modelSetOutput = inspectModelSetOutput(record);
            if (!modelSetOutput || modelSetOutput.modelClass === "unknown")
                continue;
            if (!modelSetOutput.textPreview.toLowerCase().includes("saved as your default for new sessions"))
                continue;
            const timestampMs = modelSetOutput.timestampMs;
            if (timestampMs === null || timestampMs > beforeMs + 1000)
                continue;
...
function isTaskNotificationInput(input) {
