...
export function readCodexTurnLogErrors(query, env = process.env) {
    const bounds = normalizeQueryBounds(query);
    if (!bounds || !query.sessionId)
        return [];
    let database = null;
    try {
      ...
      LIMIT ?
    `).all(TURN_ERROR_MARKER, TURN_ERROR_MARKER, MESSAGE_LIMIT, query.sessionId, bounds.startedAtSeconds, bounds.endedAtSeconds, TURN_ERROR_TARGET, TURN_ERROR_MARKER, QUERY_LIMIT);
        const seen = new Set();
        const messages = [];
...
function normalizeQueryBounds(query) {
    if (!isFiniteTimestamp(query.startedAtMs) || !isFiniteTimestamp(query.endedAtMs))
        return null;
    if (query.endedAtMs < query.startedAtMs)
        return null;
