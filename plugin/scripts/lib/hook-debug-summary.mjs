import { classifyError, classifyModel, createErrorHint, extractErrorStatusCode } from "./policy.mjs";
import { collectModelCandidates, getTranscriptPath } from "./hook-transcript.mjs";
...
export function summarizeTurnState(turn) {
    if (!turn)
        return null;
    return {
...
export function summarizePayload(payload, validationOk) {
    return {
        validationOk,
        ok: payload.ok,
